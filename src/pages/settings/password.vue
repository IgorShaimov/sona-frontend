    ...
    <card class="w-160 mx-auto">
      <div class="mx-auto py-4 lg:py-8 px-0 lg:px-12">
        <form @submit.prevent="update" @keydown="form.onKeydown($event)">
          ...
          <!-- Password -->
          <div class="mb-6">
            <label class="font-bold text-gray-700 select-none" for="password"
              >Password</label
            >
            <input
              id="password"
          ...
          <div class="mb-8">
            <label
              class="font-bold text-gray-700 select-none"
  ...
  methods: {
    async update() {
      await this.form.patch(
        `${process.env.VUE_APP_API_URL}/api/settings/password`
      )
