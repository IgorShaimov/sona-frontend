    ...
    <card class="w-160 mx-auto">
      <div class="mx-auto py-4 lg:py-8 px-0 lg:px-12">
        <form @submit.prevent="update" @keydown="form.onKeydown($event)">
          ...
          <!-- Name -->
          <div class="mb-6">
            <label class="font-bold text-gray-700 select-none" for="name"
              >Name</label
            >
            <input
              id="name"
          ...
          <!-- Email -->
          <div class="mb-8">
            <label class="font-bold text-gray-700 select-none" for="email"
              >Email</label
            >
            <input
              id="email"
  ...
  methods: {
    async update() {
      const { data } = await this.form.patch(
        `${process.env.VUE_APP_API_URL}/api/settings/profile`
      )
